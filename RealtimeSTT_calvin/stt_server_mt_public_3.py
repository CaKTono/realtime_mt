    ...
    try:
        async for msg in ws:
            if msg.type == WSMsgType.TEXT:
                ...
                # The rest of your logic for handling commands remains the same
                try:
                    ...
                    if command == "set_parameter":
                        ...
                        if lang_code == "":
                            ...
                            await ws.send_json({"status": "success", "message": f"Translation target set to {lang_code}"})
                    else:
...
async def broadcast_audio_messages(log_filename=None):
    ...
    try:
        ...
        while True:
            ...
            # print(f"{bcolors.OKCYAN}Broadcasting message: {final_message_str}{bcolors.ENDC}")

            # Make a copy of the connections to safely iterate over
            connections_to_broadcast = list(data_connections)
            if not connections_to_broadcast:
                print(f"{bcolors.WARNING}No clients connected, message not sent.{bcolors.ENDC}")

            for conn in connections_to_broadcast:
                try:
                    # --- USE EXPLICIT send_str METHOD ---
                    await conn.send_str(final_message_str)
                except Exception as e:
...
async def translation_processor_task(executor, loop):
    ...
    while True:
        ...
        # --- START MODIFICATION --- # mk4
        # When translation is enabled, print both original and translated text.
        timestamp = datetime.now().strftime('%H:%M:%S.%f')[:-3]
        original_text = job['text']

        if job['type'] == 'realtime':
            # For real-time, use carriage return to create a two-line, overwriting block.
        ...
        elif job['type'] == 'fullSentence':
            # For full sentences, print transcription and translation on separate, clean lines.
            print(f"\r[{timestamp}] {bcolors.BOLD}Sentence:{bcolors.ENDC} {bcolors.OKGREEN}{original_text}{bcolors.ENDC}")
            print(f"[{timestamp}] {bcolors.BOLD}Translation:{bcolors.ENDC} {bcolors.OKBLUE}{translated_text}{bcolors.ENDC}\n")
        # --- END MODIFICATION ---
...
async def main_async():            
    ...
    await runner.setup()
    web_server_port = 8888 # Define the port for the web server
    ...
    try:
        ...
        # Keep the server running by awaiting the broadcast task (or just sleeping indefinitely)
