...
def parse_arguments():
    ...
    parser.add_argument('-r', '--rt-model', '--realtime_model_type', type=str, default='large-v3',
...
def _recorder_thread(loop):
    ...
    global recorder, stop_recorder, pending_audio_buffer, transcription_index # Add pending_audio_buffer, transcription_index here
    ...
    def process_text(full_sentence):
        ...
        audio_filename = None
        if write_sentence_audio and pending_audio_buffer is not None and pending_audio_buffer.size > 0:
            # Check for numpy array and size, or for bytes and length
            ...
            if is_valid_buffer:
                # Increment the index for this transcription
                ...
                try:
                    ...
                    if isinstance(pending_audio_buffer, np.ndarray):
                        ...
                        if pending_audio_buffer.dtype == np.float32 or pending_audio_buffer.dtype == np.float64:
                            ...
                            # It's already int16, just convert to bytes
                            audio_bytes_to_write = pending_audio_buffer.tobytes()
                        else:
                            # It's some other NumPy type, try a direct conversion as a last resort
                    ...
                    if audio_bytes_to_write:
                        with wave.open(audio_filename, 'wb') as wf:
                            ...
                            wf.setsampwidth(pyaudio.get_sample_size(FORMAT)) # 2 bytes
                            wf.setframerate(16000) # The recorder provides audio at 16kHz
                            wf.writeframes(audio_bytes_to_write)
                        
                        if extended_logging:
                            print(f"  [{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] {bcolors.OKGREEN}Saved audio to: {audio_filename}{bcolors.ENDC}\n", flush=True, end="")
                    else:
                        print(f"{bcolors.FAIL}Could not determine how to convert audio buffer to bytes for saving.{bcolors.ENDC}")


                except Exception as e:
                    print(f"{bcolors.FAIL}Error saving audio file: {e}{bcolors.ENDC}")
                    audio_filename = f"ERROR: {e}"
...
async def main_async():            
    global stop_recorder, recorder_config, global_args, translation_manager, target_translation_language
    ...
    if args.enable_translation:
        ...
        # Create a thread pool for our blocking translation tasks
        translation_executor = ThreadPoolExecutor(max_workers=1) # 4 concurrent translations

        # Start the task that processes the translation queue
        asyncio.create_task(translation_processor_task(translation_executor, loop))
