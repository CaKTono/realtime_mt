...
# --- NEW: Global state for real-time word processing ---
realtime_word_buffer = []
last_realtime_text = ""
# ----------------------------------------------------
...
def text_detected(text, loop):
    ...
    global prev_text, last_realtime_text, realtime_word_buffer
    
    text = preprocess_text(text)

    # --- LOGIC WITH DEBUGGING FOR REAL-TIME WORD PROCESSING ---
    new_text_segment = ""
    # Check if the new text is an extension of the last one
    if text.startswith(last_realtime_text):
        new_text_segment = text[len(last_realtime_text):].strip()
        debug_print(f"Extension detected. New segment: '{new_text_segment}'")
    else:
        # The transcription model has likely revised the whole sentence.
        debug_print(f"Revision detected. Old: '{last_realtime_text}', New: '{text}'. Resetting buffer.")
        realtime_word_buffer.clear()
        new_text_segment = text.strip()

    if new_text_segment:
        new_words = new_text_segment.split()
        for word in new_words:
            realtime_word_buffer.append(word)
            # !!! THIS IS WHERE YOU TRIGGER YOUR TRANSLATION !!!
            # Example: translate_and_forward(word)
            print(f"  [REALTIME WORD]: {word}")
            debug_print(f"Word buffer state: {realtime_word_buffer}")
...
def process_text(full_sentence):
    ...
    # --- Clear buffers to prepare for the next utterance ---
    debug_print(f"Final sentence received. Clearing realtime_word_buffer and last_realtime_text.")
    last_realtime_text = ""
    print(f"\n  [FINAL BUFFER]: {' '.join(realtime_word_buffer)}")
    realtime_word_buffer.clear()
    # --- END ---
    ...
    # The full, original sentence is now sent as a single message.
    message = json.dumps({'type': 'fullSentence', 'text': full_sentence})
    # --- THE FIX: Use the global 'main_loop' to resolve the NameError ---
    asyncio.run_coroutine_threadsafe(audio_queue.put(message), main_loop)
    # --- END ---

    timestamp = datetime.now().strftime('%H:%M:%S.%f')[:-3]
    print(f"[{timestamp}] Original Sentence: {bcolors.OKGREEN}{full_sentence}{bcolors.ENDC}")
...
async def broadcast_audio_messages():
    ...
    while True:
        ...
        if data_connections:
            # Using asyncio.gather is the modern way, resolving the DeprecationWarning
...
async def main_async():
    ...
    main_loop = loop # Set the global main_loop
